...
output_script_name=script.jq

help()
{
  THIS_SCRIPT_NAME=$(basename "$0")
  echo "Creates jq script with shebang line and 755 permissions" >&2
...
jq -nr '"#!/usr/bin/jq -f"' | install --mod=755 /dev/stdin "${output_script_name}"
